...
cv::Mat FrameProcessor::process(const cv::Mat& input, Mode mode, const FrameSettings& settings) const {
    ...
    if (mode == Mode::Face && !settings.faceBoxes.empty()) {
        drawFaces(frame, settings.faceBoxes);
    }
...
void FrameProcessor::drawFaces(cv::Mat& frame, const std::vector<cv::Rect>& faces) {
...
void FrameProcessor::drawHud(cv::Mat& frame, const FrameSettings& settings, Mode mode) {
    ...
    const std::string facesText = "Faces: " + std::to_string(settings.faceBoxes.size());

    cv::rectangle(frame, cv::Rect(10, 10, 360, 140), cv::Scalar(0, 0, 0), cv::FILLED);
    ...
    cv::putText(frame, facesText, cv::Point(20, 140), cv::FONT_HERSHEY_SIMPLEX, 0.6, cv::Scalar(255, 255, 255), 2);
}
